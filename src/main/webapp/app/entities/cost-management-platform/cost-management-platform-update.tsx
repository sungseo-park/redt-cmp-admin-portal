...
import { getEntities as getHBs } from 'app/entities/hb/hb.reducer';
...
export class CostManagementPlatformUpdate extends React.Component<ICostManagementPlatformUpdateProps, ICostManagementPlatformUpdateState> {
  ...
  render() {
    const { costManagementPlatformEntity, hBS, starGates, loading, updating } = this.props;
          ...
          <Col md="8">
            {loading ? (
              ...
              <AvForm model={isNew ? {} : costManagementPlatformEntity} onSubmit={this.saveEntity}>
                ...
                <AvGroup>
                  <Label for="cost-management-platform-hb">
                    <Translate contentKey="cmpAdminPortalApp.costManagementPlatform.hb">Hb</Translate>
                  </Label>
                  <AvInput id="cost-management-platform-hb" type="select" className="form-control" name="hb.id">
                    <option value="" key="0" />
                    {hBS
                      ? hBS.map(otherEntity => (
